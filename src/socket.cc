...
#ifdef testaaa
static int socketProgressOpt(int op, struct ncclSocket* sock, void* ptr, int size, int* offset, int block, int* closed) {
  int bytes = 0;
  *closed = 0;
  char* data = (char*)ptr;
  ...
  do {
    ...
    if (bytes == -1) {
      if (errno != EINTR && errno != EWOULDBLOCK && errno != EAGAIN) {
        printf("socketProgressOpt: Call to recv from %s failed : %s", ncclSocketToString(&sock->addr, line), strerror(errno));
...
static int socketProgress(int op, struct ncclSocket* sock, void* ptr, int size, int* offset) {
  int closed;
  NCCLCHECK(socketProgressOpt(op, sock, ptr, size, offset, 0 /*block*/, &closed));
  if (closed) {
    char line[SOCKET_NAME_MAXLEN+1];
    printf("socketProgress: Connection closed by remote peer %s", ncclSocketToString(&sock->addr, line, 0));
    return ncclRemoteError;
  }
  return ncclSuccess;
}

static int socketWait(int op, struct ncclSocket* sock, void* ptr, int size, int* offset) {
  while (*offset < size)
...
static bool matchSubnet(struct ifaddrs local_if, union ncclSocketAddress* remote) {
  ...
  if (family == AF_INET) {
    ...
    printf("Net : Unsupported address family type");
    return false;
  }
}
...
int ncclFindInterfaceMatchSubnet(char* ifNames, union ncclSocketAddress* localAddrs, union ncclSocketAddress* remoteAddr, int ifNameMaxSize, int maxIfs) {
  ...
  if (found == 0) {
    printf("Net : No interface found in the same subnet as remote address %s", ncclSocketToString(remoteAddr, line_a));
  }
  freeifaddrs(interfaces);
  return found;
}
...
int ncclFindInterfaces(char* ifNames, union ncclSocketAddress *ifAddrs, int ifNameMaxSize, int maxIfs) {
  ...
  const char* env = NCCL_SOCKET_IFNAME;
  if (env && strlen(env) > 1) {
    ...
    if (nIfs == 0) {
      const char* commId = NCCL_COMM_ID;
...
#ifdef testaaa
int ncclSocketListen(struct ncclSocket* sock) {
  if (sock == NULL) {
    printf("ncclSocketListen: pass NULL socket");
    return -1;
  }
  if (sock->fd == -1) {
    printf("ncclSocketListen: file descriptor is -1");
    return -1;
  }
...
int ncclSocketGetAddr(struct ncclSocket* sock, union ncclSocketAddress* addr) {
  if (sock == NULL) {
    printf("ncclSocketGetAddr: pass NULL socket");
    return -1;
  }
...
static int socketTryAccept(struct ncclSocket* sock) {
  socklen_t socklen = sizeof(union ncclSocketAddress);
  sock->fd = accept(sock->acceptFd, &sock->addr.sa, &socklen);
  if (sock->fd != -1) {
    sock->state = ncclSocketStateAccepted;
  } else if (errno != EAGAIN && errno != EWOULDBLOCK) {
    printf("socketTryAccept: Accept failed: %s", strerror(errno));
    return ncclSystemError;
  }
  return ncclSuccess;
}

static int socketFinalizeAccept(struct ncclSocket* sock) {
  uint64_t magic;
  ...
  if (magic != sock->magic) {
    printf("socketFinalizeAccept: wrong magic %lx != %lx", magic, sock->magic);
    close(sock->fd);
    sock->fd = -1;
    ...
    if (type != sock->type) {
      printf("socketFinalizeAccept: wrong type %d != %d", type, sock->type);
...
static int socketStartConnect(struct ncclSocket* sock) {
  ...
  if (ret == 0) {
    ...
    if (++sock->refusedRetries == RETRY_REFUSED_TIMES) {
      sock->state = ncclSocketStateError;
      printf("socketStartConnect: exceeded retries (%d)", sock->refusedRetries);
      return ncclRemoteError;
    }
    usleep(SLEEP_INT);
    ...
    if (++sock->timedOutRetries == RETRY_TIMEDOUT_TIMES) {
      sock->state = ncclSocketStateError;
      printf("socketStartConnect: exceeded timeouts (%d)", sock->timedOutRetries);
      return ncclRemoteError;
    }
    usleep(SLEEP_INT);
    return ncclSuccess;
    ...
    char line[SOCKET_NAME_MAXLEN+1];
    sock->state = ncclSocketStateError;
    printf("socketStartConnect: Connect to %s failed : %s", ncclSocketToString(&sock->addr, line), strerror(errno));
    return ncclSystemError;
  }
}

static int socketPollConnect(struct ncclSocket* sock) {
  struct pollfd pfd;
  int timeout = 1, ret;
  ...
  if (ret == 0 || (ret < 0 && errno == EINTR)) {
    ...
    printf("socketPollConnect poll() failed with error %s", strerror(errno));
    return ncclRemoteError;
  } else {
  ...
  if (ret == 0) {
    ...
    if (++sock->refusedRetries == RETRY_REFUSED_TIMES) {
      sock->state = ncclSocketStateError;
      printf("socketPollConnect: exceeded retries (%d)", sock->refusedRetries);
      return ncclRemoteError;
    }
    if (sock->refusedRetries % 1000 == 0) INFO(NCCL_ALL, "Call to connect returned %s, retrying", strerror(errno));
    usleep(SLEEP_INT);
    ...
    if (++sock->timedOutRetries == RETRY_TIMEDOUT_TIMES) {
      sock->state = ncclSocketStateError;
      printf("socketPollConnect: exceeded timeouts (%d)", sock->timedOutRetries);
      return ncclRemoteError;
    }
    usleep(SLEEP_INT);
    sock->state = ncclSocketStateConnecting;
    ...
    sock->state = ncclSocketStateError;
    char line[SOCKET_NAME_MAXLEN+1];
    printf("socketPollConnect: Connect to %s returned %d(%s) errno %d(%s)", ncclSocketToString(&sock->addr, line), ret, strerror(ret), errno, strerror(errno));
    return ncclSystemError;
  }
  return ncclSuccess;
}

int ncclSocketPollConnect(struct ncclSocket* sock) {
  if (sock == NULL) {
    printf("ncclSocketPollConnect: pass NULL socket");
    return -1;
  }
...
static int socketFinalizeConnect(struct ncclSocket* sock) {
  int sent = 0;
...
static int socketProgressState(struct ncclSocket* sock) {
...
int ncclSocketReady(struct ncclSocket* sock, int *running) {
  if (sock == NULL) {
    *running = 0;
    return ncclSuccess;
  }
  if (sock->state == ncclSocketStateError || sock->state == ncclSocketStateClosed) {
    printf("ncclSocketReady: unexpected socket state %d", sock->state);
    return ncclRemoteError;
  }
...
int ncclSocketConnect(struct ncclSocket* sock) {
#ifdef ENABLE_TRACE
  char line[SOCKET_NAME_MAXLEN+1];
#endif
  const int one = 1;
  ...
  if (sock == NULL) {
    printf("ncclSocketConnect: pass NULL socket");
    return -1;
  }
  if (sock->fd == -1) {
    printf("ncclSocketConnect: file descriptor is -1");
    return -1;
  }

  if (sock->state != ncclSocketStateInitialized) {
    printf("ncclSocketConnect: wrong socket state %d", sock->state);
...
int ncclSocketAccept(struct ncclSocket* sock, struct ncclSocket* listenSock) {
  int ret = ncclSuccess;

  if (listenSock == NULL || sock == NULL) {
    printf("ncclSocketAccept: pass NULL socket");
    ret = -1;
    goto exit;
  }
  if (listenSock->state != ncclSocketStateReady) {
    printf("ncclSocketAccept: wrong socket state %d", listenSock->state);
  ...
  switch (sock->state) {
    ...
    case ncclSocketStateError:
      ...
      break;
    default:
      printf("ncclSocketAccept: wrong socket state %d", sock->state);
      ret = ncclInternalError;
      break;
  }
...
int ncclSocketInit(struct ncclSocket* sock, union ncclSocketAddress* addr, uint64_t magic, enum ncclSocketType type, volatile uint32_t* abortFlag, int asyncFlag) {
  int ret = ncclSuccess;
  ...
  if (addr) {
    ...
    if (family != AF_INET && family != AF_INET6) {
      char line[SOCKET_NAME_MAXLEN+1];
      printf("ncclSocketInit: connecting to address %s with family %d is neither AF_INET(%d) nor AF_INET6(%d)",
    ...
    if (sock->fd == -1) {
      printf("ncclSocketInit: Socket creation failed : %s", strerror(errno));
      ret = ncclSystemError;
      goto fail;
    }
  } else {
...
int ncclSocketProgress(int op, struct ncclSocket* sock, void* ptr, int size, int* offset) {
  if (sock == NULL) {
    printf("ncclSocketProgress: pass NULL socket");
    return -1;
  }
  NCCLCHECK(socketProgress(op, sock, ptr, size, offset));
  return ncclSuccess;
...
int ncclSocketWait(int op, struct ncclSocket* sock, void* ptr, int size, int* offset) {
  if (sock == NULL) {
    printf("ncclSocketWait: pass NULL socket");
    return -1;
  }
...
int ncclSocketSend(struct ncclSocket* sock, void* ptr, int size) {
  int offset = 0;
  if (sock == NULL) {
    printf("ncclSocketSend: pass NULL socket");
    return -1;
  }
  if (sock->state != ncclSocketStateReady) {
    printf("ncclSocketSend: socket state (%d) is not ready", sock->state);
    return ncclInternalError;
  }
...
int ncclSocketRecv(struct ncclSocket* sock, void* ptr, int size) {
  int offset = 0;
  if (sock == NULL) {
    printf("ncclSocketRecv: pass NULL socket");
    return -1;
  }
  if (sock->state != ncclSocketStateReady) {
    printf("ncclSocketRecv: socket state (%d) is not ready", sock->state);
    return ncclInternalError;
  }
...
int ncclSocketSendRecv(struct ncclSocket* sendSock, void* sendPtr, int sendSize, struct ncclSocket* recvSock, void* recvPtr, int recvSize) {
  int sendOffset = 0, recvOffset = 0;
  if (sendSock == NULL || recvSock == NULL) {
    printf("ncclSocketSendRecv: invalid socket %p/%p", sendSock, recvSock);
    return ncclInternalError;
  }
  if (sendSock->state != ncclSocketStateReady || recvSock->state != ncclSocketStateReady) {
    printf("ncclSocketSendRecv: socket state (%d/%d) is not ready", sendSock->state, recvSock->state);
    return ncclInternalError;
  }
...
int ncclSocketTryRecv(struct ncclSocket* sock, void* ptr, int size, int* closed, bool blocking) {
  int offset = 0;
  if (sock == NULL) {
    printf("ncclSocketTryRecv: pass NULL socket");
    return -1;
  }
  *closed = 0;
...
int ncclSocketClose(struct ncclSocket* sock) {
  if (sock != NULL) {
    if (sock->fd >= 0) {
...
int ncclSocketGetFd(struct ncclSocket* sock, int* fd) {
  if (sock == NULL) {
    printf("ncclSocketGetFd: pass NULL socket");
    return -1;
  }
  if (fd) *fd = sock->fd;
  return ncclSuccess;
...
int ncclSocketSetFd(int fd, struct ncclSocket* sock) {
  if (sock == NULL) {
    printf("ncclSocketGetFd: pass NULL socket");
    return -1;
  }
