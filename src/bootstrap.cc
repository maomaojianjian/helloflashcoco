...
int bootstrapNetInit() {
  if (bootstrapNetInitDone == 0) { // 保证只初始化一次
...
static int getRandomData(void* buffer, size_t bytes) {
  ...
  if (bytes > 0) {
    ...
    if (buffer == NULL || fp == NULL || fread(buffer, bytes, one, fp) != one) ret = -1;
    if (fp) fclose(fp);
  }
  return ret;
}

// 将本机的addr和magic->handle
int bootstrapGetUniqueId(struct ncclBootstrapHandle* handle) {
  memset(handle, 0, sizeof(ncclBootstrapHandle));

  const char* env = NCCL_COMM_ID;
  if (env) {
    if (ncclSocketGetAddrFromString(&handle->addr, env) != 0) {
      printf("Invalid NCCL_COMM_ID, please use format: <ipv4>:<port> or [<ipv6>]:<port> or <hostname>:<port>");
      return -1;
    }
    ...
    bootstrapCreateRoot(handle, false); // 监听套接字，来自其他机器的链接请求
  }
...
static int bootstrapNetSend(struct ncclSocket* sock, void* data, int size) {
  NCCLCHECK(ncclSocketSend(sock, &size, sizeof(int)));
  NCCLCHECK(ncclSocketSend(sock, data, size));
  return ncclSuccess;
}
static int bootstrapNetRecv(struct ncclSocket* sock, void* data, int size) {
  int recvSize;
...
static int bootstrapNetSendRecv(struct ncclSocket* sendSock, void* sendData, int sendSize, struct ncclSocket* recvSock, void* recvData, int recvSize) {
  int senderRecvSize;
...
int bootstrapConnect(void* commState, int peer, int tag, struct ncclSocket* sock) {
  int ret = ncclSuccess;
...
int unexpectedEnqueue(struct bootstrapState* state, int peer, int tag, struct ncclSocket* sock) {
  // New unex
...
int unexpectedDequeue(struct bootstrapState* state, int peer, int tag, struct ncclSocket* sock, int* found) {
...
int bootstrapAccept(void* commState, int peer, int tag, struct ncclSocket* sock) {
  int ret = ncclSuccess;
  struct bootstrapState* state = (struct bootstrapState*)commState;
  int newPeer, newTag;
...
int bootstrapRingAllGather(struct ncclSocket* prevSocket, struct ncclSocket* nextSocket, int rank, int nranks, char* data, int size) {
